...
export const PublicationContent = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent, index: number) => {
    if (e.key === "ArrowRight") {
      ...
      const prevIndex =
        (index - 1 + categoriesData.length) % categoriesData.length;
  ...
  return (
    <>
      <h1 className="text-primary-900 mb-8 max-w-sm text-4xl font-bold lg:text-5xl">
        {t("title")}
      </h1>
            ...
            {categoriesData.map((tab, index) => (
              <button
                ...
                className={`cursor-pointer rounded-full px-6 py-2.5 text-sm font-medium transition-all 
                    ...
                    ${activeTab === tab.name ? "bg-teal-600 text-white" : "bg-slate-200 text-slate-700"}
                `}
              >
                {tab.name}
              </button>
      ...
      {isLoading ? (
        ...
        <div className="mt-12 grid grid-cols-1 gap-10 lg:grid-cols-2">
          ...
          {featured &&
            (() => {
              const translation =
                featured.translations?.[langIndex] ||
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
                        ...
                        <div className="flex items-center gap-1">
                          <Eye className="h-3 w-3" /> {featured.seen ?? 0}{" "}
                          Dilihat
                        </div>
          ...
          <div className="grid grid-rows-3 gap-5">
            {sideArticles.map((article) => {
              const translation =
                article.translations?.[langIndex] || article.translations?.[0];
                  ...
                  <div className="cursor-pointer overflow-hidden rounded-lg transition-shadow">
                    <div className="flex h-full flex-col items-start gap-3 md:flex-row md:items-center lg:gap-5">
                      <div className="h-52 w-full shrink-0 overflow-hidden rounded-lg md:h-38 md:w-42 lg:h-48">
                        <img
                          src={
                            article.cover || "/images/placeholder-square.png"
                          }
                          alt={translation?.title}
