...
const DetailNewsClient = ({ initialData, locale, postId }: Props) => {
  ...
  const translation =
    initialData?.translations?.find(
      (t: PostTranslation) => t.language === locale,
    ) ||
    initialData?.translations?.find(
      (t: PostTranslation) => t.language === "id",
    ) ||
  ...
  const handleDownload = () => {
    const pdfAttachment = translation?.attachments?.find(
      (item) => item.type === "pdf",
    );
  ...
  const handleAddComment = async (values: any) => {
    try {
      ...
      const res = await apiRequest.post(
        `${API_ENDPOINTS.posts}/${postId}/comment`,
        payload,
      );
      ...
      <div className="container py-16">
        <div className="flex w-full flex-col items-start justify-center gap-10 xl:flex-row">
          <div className="flex w-full flex-col gap-8">
            ...
            {(() => {
              const audioFile = translation?.attachments?.find(
