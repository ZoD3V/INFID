...
export default function OrganizationStructure({
  ...
  const [selectedPerson, setSelectedPerson] =
    useState<OrganizationPeople | null>(null);
  ...
  return (
    <div className="py-20">
      <div className="container text-center">
        <h2 className="text-primary-500 mb-8 text-3xl font-bold md:text-4xl">
          {formatLabel(activeTitle)}
        </h2>
          ...
          <div className="flex w-full flex-col items-center">
            <div className="flex flex-wrap justify-center gap-6">
              {data.map((person: OrganizationPeople) => (
                <div
                  key={person.id}
                  className="flex max-w-55 shrink-0 grow-0 basis-55 justify-center"
                >
                  <MemberCard
                    name={person.name}
            ...
            <>
              ...
              <div className="flex flex-col items-center gap-5 md:flex-row md:items-start">
                <div className="mb-4 h-60 w-60 shrink-0 overflow-hidden rounded-lg bg-slate-100">
                  <img
                    src={
                      selectedPerson.image || "/images/placeholder-square.png"
                    }
                ...
                <div className="flex flex-1 flex-col items-center md:items-start">
                  ...
                  <p className="mt-4 text-center leading-relaxed text-gray-700 md:text-left">
                    {selectedPerson.description || "No profile description."}
                  </p>

                  {(selectedPerson.email || selectedPerson.phone) && (
                    <div className="mt-4 flex flex-col gap-1 text-sm text-slate-500">
                      {selectedPerson.email && (
                        <span>Email: {selectedPerson.email}</span>
                      )}
                      {selectedPerson.phone && (
                        <span>Telp: {selectedPerson.phone}</span>
                      )}
                    </div>
                  )}
                </div>
...
function MemberCard({
  ...
  return (
    ...
    >
      ...
      <p className="text-primary-500 mb-2 max-w-75 text-center text-sm font-medium">
        {role}
      </p>
    </div>
  );
